(function() {
    ...
    app.directive('flashMessage', ['$compile', '$rootScope', function($compile, $rootScope) {
        return {
            restrict: 'A',
            template: '<div role="alert" ng-show="$root.hasFlash" class="alert {{$root.flash.addClass}} alert-{{$root.flash.type}} alert-dismissible ng-hide alertIn alertOut "> <span dynamic="$root.flash.text"></span> <button type="button" class="close" close-flash><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button> </div>',
            link: function(scope, ele, attrs) {
